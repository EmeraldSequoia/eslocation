...
# This is the one, true script.  Forsake all others and follow me.
...
sub writeTwoCharNames {
    ...
    foreach my $name (@names) {
        (length $name) == 2
          or die "Country code '$name' with length != 2\n";
        print NAMES $name;  # No null, no space.  Will be read as short.
    }
    close NAMES;
}
...
writeTwoCharNames \%countryIndexHash, "$locationDir/loc-ccCodes.dat";
